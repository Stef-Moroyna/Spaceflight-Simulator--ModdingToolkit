...
namespace SFS.Parts
{
    public static class Part_Utility
    {
        ...
        static PartHit[] GetHighestDepthAtPoint(Vector2 worldPoint, PartHit[] hits, float threshold)
        {
            ...
            foreach (PartHit hit in hits)
            {
                foreach (PolygonData shape in hit.part.GetModules<PolygonData>())
                {
                    ...
                    hit.region.Raycast(shape.transform.InverseTransformPoint(worldPoint), out float depth);
        ...
        public static void GetClosestPartToClick(Vector2 point, PartHit[] hits, out float bestDistance, out PartHit bestHit)
        {
            ...
            foreach (PartHit hit in hits)
            {
                float newDistance = Polygon.GetDistanceToPolygons(point, hit.region.polygon.GetConvexPolygonsWorld(hit.part.transform));
        ...
        public static bool GetFramingBounds_WorldSpace(out Rect bounds, Part part)
        {
            ...
            return GetBounds_WorldSpace(out bounds, part.GetModules<PolygonData>().Where(x => x.BuildCollider)
                .Concat(part.GetModules<FramingColliderBounds>().Select(y => y.shape)).ToArray());
        }
        
        // Collider bounds
        public static bool GetBuildColliderBounds_WorldSpace(out Rect bounds, bool useLaunchBounds, params Part[] parts)
        {
            return GetBounds_WorldSpace(out bounds, polygon => polygon.BuildCollider, useLaunchBounds, false, parts);
        }
        ...
        static void ExpandToFitPoint(ref Vector2 min, ref Vector2 max, params Vector2[] points)
        {
        ...
        // Modules
        // public static T[] GetModules<T>(params Part[] parts)
        // {
        //     List<T> modules = new List<T>();
        //
        //     foreach (Part part in parts)
        //         modules.AddRange(part.GetModules<T>());
        //
    ...
    {
        public PartHit(Part part, PolygonData region)
        {
            this.part = part;
            this.region = region;
        }
